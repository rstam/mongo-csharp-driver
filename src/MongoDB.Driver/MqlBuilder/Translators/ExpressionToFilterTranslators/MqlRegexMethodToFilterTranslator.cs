...
{
    internal static class MqlRegexMethodToFilterTranslator
    {
        public static AstFilter Translate(MqlTranslationContext context, MethodCallExpression expression)
        {
            ...
            if (method.Is(MqlMethod.Regex))
            {
                var fieldExpression = arguments[0];
                var patternExpression = arguments[1];
                var optionsExpression = arguments[2];
                var field = MqlExpressionToFilterFieldTranslator.Translate(context, fieldExpression);
                var pattern = MqlExpressionToConstantTranslator.Translate<string>(patternExpression, expression);
                var options = MqlExpressionToConstantTranslator.Translate<string>(optionsExpression, expression);

                return AstFilter.Regex(field, pattern, options);
            }
