...
namespace MongoDB.Driver.MqlBuilder.Translators.Context
{
    internal class MqlSymbol
    {
        ...
        // constructors
        public MqlSymbol(ParameterExpression parameter, string name, AstExpression ast, IBsonSerializer serializer, bool isCurrent)
        {
        ...
        // public methods
        public MqlSymbol AsNotCurrent()
        {
            if (_isCurrent)
            {
                if (_ast is AstVarExpression varExpression)
                {
                    return new MqlSymbol(_parameter, _name, varExpression.AsNotCurrent(), _serializer, isCurrent: false);
                }
                else
                {
                    return new MqlSymbol(_parameter, _name, _ast, _serializer, isCurrent: false);
                }
            }
