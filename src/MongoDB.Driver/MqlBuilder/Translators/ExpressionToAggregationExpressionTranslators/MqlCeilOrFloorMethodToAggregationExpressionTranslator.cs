...
{
    internal class MqlCeilOrFloorMethodToAggregationExpressionTranslator
    {
        public static MqlAggregationExpression Translate(MqlTranslationContext context, MethodCallExpression expression)
        {
            ...
            if (method.IsOneOf(MqlMethod.Ceil, MqlMethod.Floor))
            {
                ...
                var ast = method.Is(MqlMethod.Ceil) ? AstExpression.Ceil(valueTranslation.Ast) : AstExpression.Floor(valueTranslation.Ast);
                return new MqlAggregationExpression(expression, ast, Int64Serializer.Instance);
            }
