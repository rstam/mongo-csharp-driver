...
{
    internal class MqlInMethodToAggregationExpressionTranslator
    {
        public static MqlAggregationExpression Translate(MqlTranslationContext context, MethodCallExpression expression)
        {
            ...
            if (method.Is(MqlMethod.In))
            {
                ...
                var value = valueTranslation.Ast;

                var valuesExpression = args[1];
                var valuesTranslation = MqlExpressionToAggregationExpressionTranslator.Translate(context, valuesExpression);
                var values = valuesTranslation.Ast;

                var ast = AstExpression.In(value, values);
                return new MqlAggregationExpression(expression, ast, BooleanSerializer.Instance);
            }
