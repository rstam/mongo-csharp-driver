...
{
    internal class MqlParameterToAggregationExpressionTranslator
    {
        public static MqlAggregationExpression Translate(MqlTranslationContext context, ParameterExpression expression)
        {
            var symbolTable = context.SymbolTable;
            if (symbolTable.TryGetSymbol(expression, out var symbol))
            {
                return new MqlAggregationExpression(expression, symbol.Ast, symbol.Serializer);
            }
