...
namespace MongoDB.Driver.Linq.Linq3Implementation.Translators.ExpressionToAggregationExpressionTranslators
{
    internal static class NewDictionaryExpressionToAggregationExpressionTranslator
    {
        ...
        public static TranslatedExpression Translate(TranslationContext context, NewExpression expression)
        {
            ...
            if (itemSerializer is IRepresentationConfigurable { Representation: BsonType.Array })
            {
                if (!(itemSerializer is IKeyValuePairSerializer keyValuePairSerializer))
                {
                    throw new ExpressionNotSupportedException(expression, because: $"item serializer type {itemSerializer.GetType()} does not implement {nameof(IKeyValuePairSerializer)}");
                }
            ...
            else if (itemSerializer is IBsonDocumentSerializer itemDocumentSerializer)
            {
