...
namespace MongoDB.Driver.Linq.Linq3Implementation.Translators.ExpressionToExecutableQueryTranslators
{
    internal static class ExpressionToExecutableQueryTranslator
    {
        ...
        public static ExecutableQuery<TDocument, IAsyncCursor<TOutput>> Translate<TDocument, TOutput>(MongoQueryProvider<TDocument> provider, Expression expression)
        {
            ...
            var knownSerializersRegistry = KnownSerializerFinder<TDocument>.FindKnownSerializers(expression, (IBsonDocumentSerializer)provider.CollectionDocumentSerializer);
