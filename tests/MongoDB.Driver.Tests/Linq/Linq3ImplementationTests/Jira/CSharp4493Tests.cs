...
namespace MongoDB.Driver.Tests.Linq.Linq3ImplementationTests.Jira
{
    public class CSharp4493Tests : Linq3IntegrationTest
    {
        ...
        public void Find_with_predicate_should_work()
        {
            ...
            translatedFilter.Should().Be("{ }"); // TODO: determine what translated filter should be
        ...
        [Fact]
        public void Minimal_repro()
        {
            var collection = CreateCollection();
            var filter1 = new JsonFilterDefinition<Customer>("{ $expr : { $eq : ['$_id', 1] } }");
            var filter2 = new JsonFilterDefinition<Customer>("{ $expr : { $ne : ['$Address.FullAddress', ''] } }");

            var filter = filter1 & filter2;

            var serializerRegistry = BsonSerializer.SerializerRegistry;
            var documentSerializer = collection.DocumentSerializer;
            var translatedFilter = filter.Render(documentSerializer, serializerRegistry, LinqProvider.V3);
            translatedFilter.Should().Be("{ $and : [{ $expr : { $eq : ['$_id', 1] } }, { $expr : { $ne : ['$Address.FullAddress', ''] } }] }");
        }
